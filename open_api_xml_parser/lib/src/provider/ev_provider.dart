...
import 'package:open_api_xml_parser/src/repository/ev_repositiory.dart';
...
class EvProvider extends ChangeNotifier {
  ...
  loadEvs() async {
    ...
    // listEvs에 _evs를 바로 작성해도 되지만 예외 처리가 추가적인 가공을 위해 나눠서 작성한다.
    List<Ev>? listEvs = await _evRepository.loadEvs();
    _evs = listEvs!;
    notifyListeners();  // 데이터가 업데이트가 됐으면 구독자에세 알린다.
  }
}