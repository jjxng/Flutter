...
//   p1.y = 5;
//   p1.z = 6;
//   // 자식 클래스(PointDetail)를 부모클래스(Point)로 타입 캐스팅 하는 것을
//   // 업캐스팅(up-casting)이라 한다.
//   // 부모 타입으로 캐스팅해서 막 사용하다가 자식 객체를 다시 받아야 할 때
//   Point p2 = p1;
//   // ...
//
//   // 다시 point detail의 z값에 접근하거나 사용해야 할 때
//   if(p2 is PointDetail)   // is로 판단해서 한 번 확인했기 때문에 as 생략가능
//     {
...
void main()
{
  ...
  // is는 특정타입이 맞다면 즉, 상속관계가가 있으면 true를 반환한다.
  // 원래 클래스를 확인하는 것
  if (unit1 is Marine) {
    print('마린입니다.');
  ...
  */
  // print(unit1.name); => 단독적으로는 해당문장을 사용할 수 없다.
  /*
    이 문장을 if문 블럭 외부로 옮기면 에러가 발생한다.
    is를 통해 부모타입을 자식 타입으로 변환이 가능한지 확인한 후 접근을 허용하게 된다.
  */
