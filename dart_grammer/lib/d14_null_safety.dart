void main()
{
  ...
  // *** Null Safety 규칙 *** null일 때 안전하게 사용하기 위해
  ...
  // int 타입으로 유추. Non-Nullable 변수임으로 null이 대입 불가능
  var a3 = 10;
  ...
  // Null이 아닐 때 메서드 실행
  // ?를 제거하면 에러발생. 객체가 null인 상태에서는 멤버메서드를 호출할 수 없기 때문이다.
  name = name?.toLowerCase();

  // Null일 때
  // n42는 정수가 아니므로 정수로 변환 시 에러 발생됨. 따라사ㅓ val2는 -1로 초기화 된다.
  ...
  // name1 = 'ssss';    // 이 값이 없으면 런타임 에러가 난다.
    ...
    // String text = name1;
    // 그럼 저렇게 에러가 떴을 때 어떻게 해야하나 하면, 변수 뒤에 '!'를 붙여주면 된다.
    // 그러면 플러터는 '아 저 변수는 사용되는 시점에서 null값이 절대 아니구나'하고 에러 없어짐.
    String text = name1!;
    print(text);
}