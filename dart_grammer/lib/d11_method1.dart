void main()
{
  ...
  //                          따라서 순서를 변경할 수 있다. set{}처럼 키 값으로 찾아감.

  // 파라미터를 옵션 처리하기
  something3(); // 선택형 매개변수이기 때문에
  // 이렇게 있는 것은 써도 되고 안 써도 됨.
  // 뒤의 파라미터만 이름을 주어 값을 전할 수 있다.
  something3(name: '손오공');

  // 옵션이 있는 것은 아래와 같이 사용하면 된다. 하나만 사용해도 되고, 둘 다 사용해도 된.
  // 형식은 (key : value)만 지키면 된다. 순서를 지키지 않아도 된다. {}이므로

  something4(age: 20);

  something5('해리포터');
  something5('멀린', age: 20);
  // something5(age: 46, name: '관우아들');  // 순서 변경 안 됨
...
/*
  매서드 오버로딩 : 오버로딩이란 메서드를 정의할 때 동일한 이름으로 정의하되 매개변수의 개수나
    타입을 다르게 해서 서로 다른 메서드처럼 정의하는 기법을 말한다.
...
// 필수 + 옵션. 필수는 반드시 사용해야 한다. {}은 사용해도 되고 안 해도 된다.
// 이렇게 헷갈리기 때문에 나중에는 다 옵션으로 많이 사용한다.
void something5(String name, {int? age}) {
  print('name : $name age : $age');
}

// 파라미터의 디폴트 값. ? 빠진다. int age = 10이기 때문에 ? 빠진다.
void something6(String name, {int age = 10}) {
  print('name : $name age : $age');
}

// 반드시 필요한 파라미터라면... 이전 방식 @required(옛날 버전) 강제로 사용.
// 생략하면 에러남.
