void main()
{
  ...
  // 실패 시 Exception : 예외가 발생하면 그 즉시 프로그램의 실헹이 중지된다.
  ...
  // 실패 시 null. tryParse에 try ~ on이 들어가 있다. 그래서 예외가 생기면 null
  int? intVal2 = int.tryParse('10');  // 10으로 변환
  // int?에서 ?는 뒤에가 tryParse가 false이면 null이 들어간다는 소리.
  double? doubleVal2 = double.tryParse('10.1'); // 10.1로 변환

  // 삼항연산자 형태로 앞부분에서 예외가 발생하면 ?? 뒷부분의 값이 할당된다.
  int val1 = int.tryParse('n42') ?? -1;
  // 여기서 int에 ?가 없는 이유 val1의 값에 -1이 들어오므로 null이 들어올 수 없음.
  print('val1 = $val1');  // 예외가 발생하므로 -1이 할당됨.

  // var는 어떤 값이라도 들어올 수 있다. 즉, null도 들어올 수 있다는 것이므로 ?를 안 써도 됨.
  var val2= int.tryParse('n42');
  print('val2 = $val2');  // 예외가 발생하므로 null이 할당 됨.
